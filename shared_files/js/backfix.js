 ...
 */
function removeAdIdFromQueryString(queryString) {
    // Parse the query string into URLSearchParams object
    let params = new URLSearchParams(queryString);

    // Remove the ad_id parameter
    if (params.has('ad_id')) {
        params.delete('ad_id');
    }
...
const backLink =
    ...
    '?' +
    removeAdIdFromQueryString(window.location.search.substring(1)) +
    '&back=true';
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    function showFrame(name) {
        ...
        document
            .querySelectorAll(`body > *:not(#${name})`)
            .forEach(function (e) {
